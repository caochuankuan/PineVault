...
状态：已实现。条目编辑支持手动输入 Base32 密钥、粘贴 `otpauth://` 链接和扫描二维码，并可设置 SHA1/SHA256/SHA512、6/8 位和 30/60 秒周期。查看面板显示当前验证码、剩余时间和复制入口。TOTP 配置随密码库 payload 加密和 WebDAV 同步，KDBX 导入导出保留兼容字段。

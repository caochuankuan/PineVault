...
当前进度：本地加密密码库闭环已经实现，并在 Android 真机通过加密、错误密码、密文篡改、持久化与重新解锁测试。密码条目、TOTP 动态验证码、收藏、多选批量操作、标签、分组筛选、首页显示开关、排序、KDBX 导入导出、坚果云 WebDAV 设置、加密凭据、远端目录初始化、新设备恢复、自动与手动同步、同步历史、ETag 自动重试、三方冲突合并、主密码修改及设备验证解锁已经实现。锁定状态按安全设计不执行同步。
...
- 更多菜单：同步、WebDAV 设置、同步历史、主密码、设备验证解锁、锁定、KDBX 导入导出、首页密码/网站/TOTP 显示和排序切换。
...
状态：已实现。条目支持标签和单个分组；首页支持按分组筛选，更多菜单支持分组重命名、删除（条目移到未分组）和拖动排序，并支持按名称/时间正序或倒序排序。固定的 TOTP 智能分组会动态汇总所有配置了验证码的条目，不写入密码库分组数据，也不可重命名、删除或排序。分组变更会进入同步合并，删除通过分组墓碑传播。长按和多选模式提供批量复制、收藏切换、移动分组和删除。标签仅在查看和编辑面板展示。
...
状态：已实现。条目编辑支持手动输入 Base32 密钥、粘贴 `otpauth://` 链接和扫描二维码，并可设置 SHA1/SHA256/SHA512、6/8 位和 30/60 秒周期。查看面板显示当前验证码、剩余时间和复制入口；进入首页的 TOTP 智能分组时会强制显示紧凑验证码行，不受全局“显示 TOTP”开关影响。TOTP 配置随密码库 payload 加密和 WebDAV 同步，KDBX 导入导出保留兼容字段。

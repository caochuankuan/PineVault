...
状态：安全同步闭环已实现。支持手动同步、解锁/恢复后同步、内容变更防抖同步、返回前台同步和解锁期间每 5 分钟同步；412 条件上传失败会重新下载并重试一次。冲突以带“同步冲突”后缀的新条目呈现，WebDAV 配置冲突保留本机配置并显示提示。同步阶段和最近 100 条历史可在界面查看。真实坚果云双设备验证仍需有效账号完成。

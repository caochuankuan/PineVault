...
当前进度：本地加密密码库闭环已经实现，并在 Android 真机通过加密、错误密码、密文篡改、持久化与重新解锁测试。密码条目、收藏、多选批量操作、标签、分组筛选、首页显示开关、排序、KDBX 导入导出、坚果云 WebDAV 设置、加密凭据、远端目录初始化、新设备恢复、自动与手动同步、同步历史、ETag 自动重试、三方冲突合并及主密码修改已经实现。锁定状态按安全设计不执行同步。
...
- 更多菜单：同步、WebDAV 设置、同步历史、主密码、锁定、KDBX 导入导出、首页密码/网站显示和排序切换。
...
状态：已实现。条目支持标签和单个分组；首页支持按分组筛选，更多菜单支持按名称/时间正序或倒序排序。长按和多选模式提供批量复制、收藏切换、移动分组和删除。标签仅在查看和编辑面板展示。

...
当前进度：本地加密密码库闭环已经实现，并在 Android 真机通过加密、错误密码、密文篡改、持久化与重新解锁测试。坚果云 WebDAV 设置、加密凭据、远端目录初始化、新设备恢复、自动与手动同步、同步历史、ETag 自动重试、三方冲突合并及主密码修改已经实现。锁定状态按安全设计不执行同步。
...
状态：安全同步闭环已实现。支持手动同步、解锁/恢复后同步、内容变更防抖同步、返回前台同步和解锁期间每 5 分钟同步；412 条件上传失败会重新下载并重试一次。冲突以带“同步冲突”后缀的新条目呈现，WebDAV 配置冲突保留本机配置并显示提示。主密码包装以远端已提交版本为准，旧主密码失败时提示用户输入其他设备设置的最新主密码。同步阶段和最近 100 条历史可在界面查看。真实坚果云双设备验证仍需有效账号完成。

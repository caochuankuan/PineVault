...
- TOTP 动态验证码：支持 Base32 密钥、`otpauth://` 链接和扫码录入，查看时显示倒计时并可一键复制
- 手机端查看面板以底部抽屉展示，按内容自适应高度，内容过多时可滚动；编辑面板保持独立滚动布局
- 每条记录支持多个标签；标签只在查看和编辑面板展示，首页卡片不显示
- 分组：新建、编辑时选择分组；更多菜单支持分组重命名、删除（记录移到未分组）和拖动排序，首页按分组筛选；固定的 TOTP 智能分组会汇总所有动态验证码条目
- 搜索：按名称、用户名或网站过滤
- 首页显示开关：密码、网站、TOTP 可分别控制
...
当前支持 Title、UserName、Password、URL、Notes、TOTP，以及分组和收藏标记。TOTP 导入兼容 `otpauth://` 和 KeePass 常见原生字段；无法解析的验证码会在预览中特殊标注，不阻止用户导入其他字段。暂不支持 KDBX 密钥文件、附件和自定义字段。
...
打开“更多 → 设备验证解锁”，输入当前主密码并完成系统身份验证即可绑定。本机之后启动应用或手动锁定时会自动弹出系统验证；取消或失败后停留在解锁页，也可以再次点击“使用设备验证解锁”。关闭功能仍在同一菜单操作，主密码始终可以作为回退方式。
